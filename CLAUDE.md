...
- **Formulario de feedback**: formulario externo en `https://arclycee-aux.web.app/form.html` (repo `arclycee-aux/`). Integrado en `juego.html` (botón flotante dorado, solo visible online), en `docs/about*.html` (sección "Contáctanos" con modal) y **en todas las páginas de docs** vía `navigation.js` (botón flotante inyectado automáticamente, bottom-right desktop, top-right mobile). Usa `if (!document.getElementById('arc-feedback-btn'))` para evitar duplicados con about.html. Pasa `?lang=` para i18n automático. Firebase Firestore backend (proyecto `cemiai`, base de datos `arclycee`)
